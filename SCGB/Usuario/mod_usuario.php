            ...
            <div class="row-fluid">
               <div class="span12">
                  ...
                  <ul class="breadcrumb">
                     ...
                     <li><a href="add_trabajador.php">Buscar Usuario</a></li>
                  </ul>
               ...
               <div class="span12">
                  <div class="widget box green">
                     <div class="widget-title">
                        <div class="widget-title">
                        <h4><i class="icon-globe"></i>Tabla de Usuario</h4>
                     </div>
                     </div>
                     <div class="widget-body form">
                     <div class="clearfix margin-bottom-10">
                           <div class="btn-group">
                              <button id="sample_editable_1_new" class="btn btn-success" onclick = "location.href = 'add_usuario.php'">
                              Agregar Usuario <i class="icon-plus"></i>
                              </button>
                           </div>
                 ...
                 <div style="display: none;" id='editarFormulario'>
                           ...
                           <input type="hidden" name="CODIGOPERMISO" id="CODIGOPERMISO">
                        <form action="" method="post" id="form2" name="form2"  class="form-horizontal">
                           <div class="control-group">
                              <label class="control-label">Login</label>
                              <div class="controls">
                                 <input type="text"  name="LOGIN" id="LOGIN" class="span6 " disabled="disabled"/>
                              </div>
                           </div>
                           ...
                           <label class="control-label">Nombre</label>
                              <div class="controls">
                            <input  type="text" class="span6" name="NOMBRE" id="NOMBRE" required/>
                              </div>
                           </div>
                            ...
                            <div class="control-group">
                              ...
                              <div class="controls">
                                 <input type="text" name="APELLIDO" id="APELLIDO" class="span6" required/>
                              </div>
                           </div>               
                        ...
                        <div class="control-group">
                              ...
                              <div class="controls">
                                  <input type="number" name="TELEFONO" id="TELEFONO"  class="span6" required/>
                              </div>
                          </div>
                           ...
                           <div class="control-group">
                              ...
                              <div class="controls">
                                  <input type="email" name="EMAIL" id="EMAIL"  class="span6" required/>
                              </div>
                           </div>
...
<script type="text/javascript" src="../js_mantenedores/jquery.msgbox.min.js"></script>
 ...
 <script>
   ...
   $("#tab1").load("tbUsuario.php");
   $("#btnFancybox").fancybox();
   
   		   $("#form2").submit(function(e){
		   CODIGOPERMISO= $('#CODIGOPERMISO').val() 
           if($("#PROVEEDOR").is(':checked')) {PROVEEDOR=1;} else {PROVEEDOR= 0;}  
		   if($("#PRODUCTO").is(':checked')) {PRODUCTO=1;} else {PRODUCTO= 0;}  
	       if($("#PERSONAL").is(':checked')) {PERSONAL=1;} else {PERSONAL= 0;}  
		   if($("#OBRA").is(':checked')) {OBRA=1;} else {OBRA= 0;} 
		   if($("#BODEGA").is(':checked')) {BODEGA=1;} else {BODEGA= 0;} 
		   if($("#INFORME").is(':checked')) {INFORME=1;} else {INFORME= 0;} 
		   if($("#ADMINISTRACION").is(':checked')) {ADMINISTRACION=1;} else {ADMINISTRACION= 0;} 
		   LOGIN= $('#LOGIN').val()
		   ...
		   EMAIL= $('#EMAIL').val()      
            ...
            $.ajax({
                ...
                data: {CODIGOPERMISO:CODIGOPERMISO,PROVEEDOR:PROVEEDOR, PRODUCTO:PRODUCTO, PERSONAL:PERSONAL, OBRA:OBRA,BODEGA:BODEGA,INFORME:INFORME,ADMINISTRACION:ADMINISTRACION, LOGIN:LOGIN, PASSWORD:PASSWORD, NOMBRE:NOMBRE, APELLIDO:APELLIDO, TELEFONO:TELEFONO, EMAIL:EMAIL},
                success: function(data){
                $.fancybox(data)
                }
            });
            $.fancybox.close();
			$.msgbox("Usuario Modificado Exitosamente",{type: "info",buttons : [{type: "submit", value: "Aceptar"}]},function(result) { if(result=="Aceptar"){window.location.href='mod_usuario.php';} });
        });
   })
   ...
   function editarUsuario(id) {
				...
				limpiaCheck();
					$("#CODIGOPERMISO").val(data[0].CODIGOPERMISO)
					...
					if(data[0].PROVEEDOR==1){document.getElementById('PROVEEDOR').click();}
					if(data[0].PRODUCTO==1){document.getElementById('PRODUCTO').click();}
					if(data[0].PERSONAL==1){document.getElementById('PERSONAL').click();}
					if(data[0].OBRA==1){document.getElementById('OBRA').click();}
					if(data[0].BODEGA==1){document.getElementById('BODEGA').click();}
					if(data[0].INFORME==1){document.getElementById('INFORME').click();}
					if(data[0].ADMINISTRACION==1){document.getElementById('ADMINISTRACION').click();}
                    $("#btnFancybox").click()
	...
	function eliminarUsuario(id,nombre) {
		  $.msgbox("¿Desea Eliminar el Usuario con Login: "+id+" Nombre: "+nombre+"?", {type: "confirm",buttons : [{type: "submit", value: "SI"},{type: "submit", value: "NO"}]}, function(result) { 
		 if(result=="SI"){
			 $.getJSON("obtUsuarioId.php", {id_usuario: id}, function(data) {
				   CODIGO= data[0].LOGIN
				   $.ajax({
                     ...
                     data: { CODIGO: CODIGO},
                     success: function(data){
					  $.msgbox("Usuario Eliminado Exitosamente",{type: "info",buttons : [{type: "submit", value: "Aceptar"}]},function(result) { if(result=="Aceptar"){window.location.href='mod_usuario.php';} });
                     }
                  });
               })
			 } 
