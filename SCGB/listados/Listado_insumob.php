...
document.write("<span class='class8' id=fecha>"+dayarray[day]+" "+daym+" de "+montharray[month]+" de "+year+"</span>")
                        ...
                        <table class="table table-advance tabs-stacked">
                           <thead>
                              <tr class="table-condensed">
								 ...
								 <th>Nombre</th>
                                 <th>Marca</th>
                                 <th class="hidden-300">Modelo</th>
                                 <th class="hidden-300">Tipo Insumo</th>
                                 <th class="hidden-300">Stock</th>
	...
	$sql = "SELECT p.CODIGOPRODUCTO,p.P_NOMBRE,p.P_MARCA,p.P_MODELO,p.P_OBSERVACION,i.I_CANTIDAD,s.S_CANTIDAD FROM insumo i,producto p,stock s ,ingreso_producto ip  WHERE p.CODIGOPRODUCTO=i.CODIGOPRODUCTO and p.CODIGOPRODUCTO=s.CODIGOPRODUCTO and p.P_ESTADO=1 and ip.CODIGOPRODUCTO =p.CODIGOPRODUCTO and IP_FECHA BETWEEN '$x' and '$y'";
	conectar();
	$rs=mysql_query($sql,$conexion);	
	while ($row=mysql_fetch_array ($rs)){
  ...
  ?>
                              <tr>
							   <td><?=$row["CODIGOPRODUCTO"];?></td>
                                 <td><?=$row["P_NOMBRE"];?></td>
                                 <td><?=$row["P_MARCA"];?> </td>
                                 <td class="hidden-480"><?=$row["P_MODELO"];?></td>
								 
                                 <td class="hidden-480"><?=$row["P_OBSERVACION"];?></td>
								 
                                 <td class="hidden-480"><?=$row["S_CANTIDAD"];?></td>
