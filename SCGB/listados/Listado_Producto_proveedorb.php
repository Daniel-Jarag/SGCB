         ...
         <div class="container-fluid">
            ...
            <div class="row-fluid hidden-print">
               <div class="span12">
                  ...
                  <h3 class="page-title">
                     Informe productos por Proveedores         
...
document.write("<span class='class8' id=fecha>"+dayarray[day]+" "+daym+" de "+montharray[month]+" de "+year+"</span>")
                          ...
                          <h1>Producto por proveedores  </h1> 
                          </span></p>
                        </div>
                     </div>
	...
	$sql1 = "SELECT distinct(p.RUT),p.PD_NOMBRE,p.PD_DIRECCION,p.PD_TELEFONO FROM proveedor p INNER JOIN ingreso_producto i
ON  p.RUT=i.RUT and p.PD_ESTADO=1";
	conectar();
	$rss=mysql_query($sql1,$conexion);	
  ...
  ?>               
             <div class="row-fluid">
                        ...
                        <div class="span12">
                           ...
                           <h4>nombre:<?=$row1["PD_NOMBRE"];?>  </h4>
                           <ul class="unstyled">
                              <li>Telefono: <?=$row1["PD_TELEFONO"];?> </li>
                              <li>Direccion:
                               
                              <?=$row1["PD_DIRECCION"];?> </li>
                           </ul>
                        </div>
                       ...
                       <table class="table table-advance tabs-stacked">
                         <thead>
                           <tr class="table-condensed">
                             ...
                             <th class="span4">Precio Unitario</th>
                         ...
                         <tbody>
                           <?
  	$codigo=$row1["RUT"];
	$sql = "SELECT p.P_NOMBRE, i.IP_CANTIDAD, i.IP_PRECIOUNITARIO, (
i.IP_CANTIDAD * i.IP_PRECIOUNITARIO
) AS total
FROM producto p
INNER JOIN ingreso_producto i ON i.RUT='$codigo' and  p.CODIGOPRODUCTO = i.CODIGOPRODUCTO 
";
	conectar();
	$rs=mysql_query($sql,$conexion);	
	while ($row=mysql_fetch_array($rs)){
