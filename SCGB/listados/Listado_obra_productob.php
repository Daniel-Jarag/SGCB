...
<body class="fixed-top">
   ...
   <div id="header" class="navbar navbar-inverse navbar-fixed-top">
     <div id="header2" class="navbar navbar-inverse navbar-fixed-top">
...
document.write("<span class='class8' id=fecha>"+dayarray[day]+" "+daym+" de "+montharray[month]+" de "+year+"</span>")
	...
	$sql1 = "SELECT distinct(p.CODIGOOBRA),o.O_NOMBRE,o.O_FECHAINICIO,o.O_FECHATERMINO FROM obra o INNER JOIN prestamo p
ON  o.CODIGOOBRA=p.CODIGOOBRA and o.O_ESTADO=1";
	conectar();
	$rss=mysql_query($sql1,$conexion);	
  ...
  ?>               
             <div class="row-fluid">
                        ...
                        <div class="span12">
                           ...
                           <h4>Nombre:<?=$row1["O_NOMBRE"];?>  </h4>
                           <ul class="unstyled">
                              <li>Fecha Inicio : <?=$row1["O_FECHAINICIO"];?> </li>
                              <li>Fecha Termino :
                               
                              <?=$row1["O_FECHATERMINO"];?> </li>
                           </ul>
                        </div>
                     ...
                     <div class="row-fluid">
                       <table class="table table-advance tabs-stacked">
                         ...
                         <tbody>
                           <?
  	$codigo=$row1["CODIGOOBRA"];
	$sql = "SELECT p.P_NOMBRE , count(dp.CODIGOPRODUCTO) as total FROM producto p,detalle_prestamo dp WHERE dp.CODIGOPRODUCTO=p.CODIGOPRODUCTO and dp.CODIGOPRESTAMO in (select pr.CODIGOPRESTAMO from prestamo pr where pr.CODIGOOBRA=$codigo) GROUP BY  p.P_NOMBRE
";
	conectar();
	$rs=mysql_query($sql,$conexion);	
	while ($row=mysql_fetch_array($rs)){
