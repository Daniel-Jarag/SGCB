...
$guarda1 = array();
$cont=0;
$res = $cn->Consulta("SELECT dp.CODIGOPRODUCTO,dp.CODIGOUNITARIO,dp.CODIGOPRESTAMO FROM detalle_prestamo dp,producto_unitario pu
 WHERE dp.CODIGOUNITARIO=pu.CODIGOUNITARIO AND CODIGOPRESTAMO='$id' AND pu.PU_ESTADO=2 AND dp.CODIGOPRODUCTO=pu.CODIGOPRODUCTO AND dp.DP_ESTADO=1");
...
while ($i < count($obtiene)) {
 ...
 WHERE CODIGOPRODUCTO='$consulta'");
	...
	if($IDENTIFICADOR=="H"){
		 $res3 = $cn3->Consulta("
SELECT p.CODIGOPRODUCTO,pu.CODIGOUNITARIO,p.P_NOMBRE,p.P_MARCA,p.P_MODELO,p.P_OBSERVACION,h.H_TIPOHERRAMIENTA,h.H_FRECUENCIA,h.H_POTENCIAMAXIMA FROM producto p,producto_unitario pu,herramienta h
 WHERE p.CODIGOPRODUCTO=pu.CODIGOPRODUCTO AND p.CODIGOPRODUCTO=h.CODIGOPRODUCTO AND p.CODIGOPRODUCTO='$consulta' AND pu.CODIGOUNITARIO='$consulta2'"); 
		 while($row3 = $cn3->getRespuesta($res3)){
		     ...
		     $res3 = $cn3->Consulta("SELECT p.CODIGOPRODUCTO,p.P_NOMBRE,p.P_MARCA,p.P_MODELO,p.P_OBSERVACION,p.P_IDENTIFICADOR,v.V_PERMISO,v.V_YEAR,v.V_CONDICION,v.V_PATENTE,pu.CODIGOUNITARIO FROM vehiculo v,producto p,producto_unitario pu 
WHERE p.CODIGOPRODUCTO=v.CODIGOPRODUCTO AND p.CODIGOPRODUCTO=pu.CODIGOPRODUCTO AND p.CODIGOPRODUCTO='$consulta' AND pu.CODIGOUNITARIO='$consulta2'"); 
		      while($row3 = $cn3->getRespuesta($res3)){
			   array_push($guarda1, $row3['CODIGOPRODUCTO']);
