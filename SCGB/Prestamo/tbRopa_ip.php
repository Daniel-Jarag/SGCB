...
$cn = new Conexion();
$cn->Conectar();
$res = $cn->Consulta("SELECT p.CODIGOPRODUCTO,pu.CODIGOUNITARIO,p.P_NOMBRE,p.P_MARCA,p.P_MODELO,p.P_OBSERVACION,p.P_IDENTIFICADOR,r.R_TALLA,r.R_COLOR,r.R_MATERIAL FROM producto p,producto_unitario pu,ropa r
WHERE p.CODIGOPRODUCTO=pu.CODIGOPRODUCTO AND p.CODIGOPRODUCTO=r.CODIGOPRODUCTO AND P_IDENTIFICADOR='R' AND p.P_ESTADO=1 AND pu.PU_ESTADO=1");
?>
<script>
    $("#tablaRopa").dataTable({
            "oLanguage": {
